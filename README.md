...
* **`doc/RAG/`**
  * 包含檢索增強生成（RAG）的進階系列教學筆記與架構圖，涵蓋了從基礎到進階的 RAG 流程，包含：**Routing & Query Construction**、**Indexing**、**Retrieval**、**Generation & Evaluation**，以及探討超越 RAG 框架的長脈絡處理 (**Scaling Long Context**) 等深入主題。
* **`doc/VLM/`**
  * 存放關於視覺語言模型（VLM）的原始 PDF 論文、PDF 轉換出的 Markdown 檔案，以及繁體中文翻譯版本。目前收錄的文獻高度聚焦於：
    * **視覺與語意定位 (Visual / Semantic Grounding)** (如 *Q-GroundCAM*, *Qwen3-VL-Seg*, *R-VLM GUI Grounding*)
    * **空間推理能力 (Spatial Reasoning)** (如 *SpaRE*, *ViewSpatial-Bench*)
    * **模型自我糾錯與視覺思考 (Self-Correction & Visual Thinking)**
* **`doc/arxiv paper search/`**
  * 包含由 ArXiv 檢索任務自動產生的最新文獻彙整報告（Markdown 格式），涵蓋以下前沿領域：
    * **RAG 最新研究回顧** (中英文版)
    * **VLM 與複雜文件解析** (如複雜 PDF/PPT 表格擷取、硬體工程文件提取、VLM 表格理解)
    * **VLM Visual Grounding 最新進展**
    * **機構設計的 AI 應用** (如 3D CAD AI Agents、工程標準自動檢查)
* **`doc/LLM/`**
  * 存放大型語言模型（LLM）相關論文的原始 PDF、Markdown 轉換稿與繁體中文翻譯，目前聚焦於 AI 輔助認知流程中的歸因謬誤等主題。
* **`doc/CAD/`**
  * 收錄與 CAD／工程設計工具整合相關的筆記，例如 Claude 與 PTC Creo Parametric 透過 MCP 整合的教學紀錄。
* **`doc/Issue-Risk-Management/`**
  * 收錄議題與風險管理相關論文的原始 PDF 與繁體中文翻譯，例如硬體開發中的錯誤追蹤與 NLP 應用研究。

## YouTube 影音字幕與翻譯 (`youtube-transcript/`)

本專案包含 `youtube-transcript/` 目錄，專門用來存放從 YouTube 頻道中抓取、解析與翻譯的影片內容（例如技術演講、教學影片）。

* **資料結構：**
  * 依據 YouTube 頻道名稱（例如 `ai-engineer/` 或 `matt-pocock/`）進行頂層分類。
  * 每個頻道目錄下，會再針對單一影片建立獨立的資料夾。
* **常見內容物：**
  * **`meta.json`**: 影片的中繼資料（如標題、頻道資訊）。
  * **`imgs/cover.jpg`**: 影片封面縮圖。
  * **原始字幕檔**: 包含時間軸與斷句的原始字幕資料（如 `transcript-raw.json`、`transcript-sentences.json`）。
  * **翻譯與重點整理**: 經過 AI 翻譯與排版的繁體中文筆記（如 `transcript-zh-TW.md`、`translation.md`）。

## 暫存工作區 (`_scratch/`)

`_scratch/` 用來存放翻譯/整理過程中的中繼檔案（草稿、chunks、原始素材等），不屬於正式文獻庫的一部分，已被 `.gitignore` 排除，不會進版本控制。
