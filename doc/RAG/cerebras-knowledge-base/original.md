...
requestedUrl: "https://www.cerebras.ai/blog/how-we-built-our-knowledge-base"
author: "Isaac Tai, Daniel Kim, Mike Gao"
coverImage: "https://cdn.sanity.io/images/e4qjo92p/production/e16846d802e8869641cca284f224e49925584c38-1200x630.png?w=1200&h=630&fit=max&auto=format"
summary: "See how Cerebras built an enterprise AI knowledge base that connects Slack, code repositories, documentation, and custom data sources."
adapter: "generic"
capturedAt: "2026-07-22T04:19:07.034Z"
...
CHECKPOINT\_LOADER.CC LANGUAGE-AWARE

12345678910111213141516171819202122232425262728293031

*class* CheckpointLoader { *public*: *Status* *LoadManifest* (*Path* p) { *auto* bytes = fs\_.Read(p); *if* (!bytes.ok()) *return* bytes.status(); manifest\_ = *ParseManifest* (\*bytes); *return* *ValidateShards* (manifest\_); } *Status* *WarmShardCache* () { *for* (*const* *auto* & shard: manifest\_.shards()) { cache\_.Pin(shard.key()); stats\_.Record(shard.bytes()); } *return* *OkStatus* (); } *Status* *Restore* (*RestoreOptions* opts) { *auto* lease = coordinator\_.Acquire(opts.cluster); *if* (!lease.ok()) *return* lease.status(); *for* (*auto* id: manifest\_.ordered\_ids()) { RETURN\_IF\_ERROR(fetcher\_.Fetch(id)); } *return* coordinator\_.Release(\*lease); } *private*: *FileSystem* fs\_; *Manifest* manifest\_; *ShardCache* cache\_;};
...
2CLASS: CHECKPOINTLOADER

3WIKI: CHECKPOINT FORMAT

4THREAD: NFS STALL

5INC-82: SLOW RESTORE

1WIKI: CHECKPOINT FORMAT

2CKPT\_LOADER.CC

3RUNBOOK: NFS MOUNTS

4INC-82: SLOW RESTORE

5THREAD: CACHE WARMUP

1THREAD: NFS STALL

2THREAD: CACHE WARMUP

3CLASS: CHECKPOINTLOADER

4WIKI: LOAD STALLS

5INC-82: SLOW RESTORE

1CLASS: CHECKPOINTLOADER
...
5CLASS: CHECKPOINTLOADER

1THREAD: NFS STALL

2THREAD: CACHE WARMUP

3BURST: TIMEOUT FLAGS
...
SCORE(D) = SUM 1 / (60 + RANK\_L(D)) | K = 60
